namespace HuaweiARUnitySDK
{
     ...
     * can always detect the guestures and the hand box around the box. However, only when front camera is choosen by 
     * \link ARHandTracking.CameraLensFacing \endlink and \link ARHandTracking.EnableDepth\endlink is true, 
     ...
     * 以及手势周边的盒子。目前，只有当\link ARHandTracking.CameraLensFacing \endlink为前置相机且
     * \link ARHandTracking.EnableDepth\endlink开启的时候，才可能识别出手的类型和手骨数据（受手机类型限制）。骨骼点的定义如下：
     * \htmlonly <style>div.image img[src="ARHand.png"]{width:400px;}</style> \endhtmlonly
     * @image html ARHand.png
    ...
    public class ARHand : ARTrackable
    {
         ...
         * @brief Get hand type.
         * 
         * Currently, only when front camera is choosen by \link ARHandTracking.CameraLensFacing \endlink
         * and \link ARHandTracking.EnableDepth\endlink is true, the hand type may be distinguished accroding to devices' 
         * differences.
         * \else
         * @brief 获取手势类型。
         * 
         * 目前，只有当\link ARHandTracking.CameraLensFacing \endlink为前置相机并且\link ARHandTracking.EnableDepth\endlink
