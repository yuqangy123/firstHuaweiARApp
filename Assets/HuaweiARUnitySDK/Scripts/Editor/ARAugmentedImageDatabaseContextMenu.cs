...
namespace HuaweiARInternal
{
    ...
    public static class AugmentedImageDatabaseContextMenu
    {
        ...
        [MenuItem("Assets/Create/HuaweiARUnitySDK/ARAugmentedImageDatabase", true, 6)]
        ...
        [MenuItem("Assets/Create/HuaweiARUnitySDK/GoogleARAugmentedImageDatabase", false, 7)]
        private static void AddAssetsToNewGoogleAugmentedImageDatabase()
        {
            var selectedImagePaths = new List<string>();
            bool unsupportedImagesSelected = false;
            ...
            {
                var newDatabase = ScriptableObject.CreateInstance<ARAugmentedImageDatabase>();
                newDatabase.SetARAugImageDatabaseType(AREnginesType.GOOGLE_AR_CORE);
                var newEntries = new List<ARAugmentedImageDatabaseEntry>();
                foreach (var imagePath in selectedImagePaths)
                {
                    var fileName = Path.GetFileName(imagePath);
        ...
        [MenuItem("Assets/Create/HuaweiARUnitySDK/ARAugmentedImageDatabase", true, 7)]
        private static bool AddAssetsToNewGoogleAugmentedImageDatabaseValidation()
        {
            bool unsupportedSelected;
            return _GetSelectedImagePaths(out unsupportedSelected).Count > 0;
        }
